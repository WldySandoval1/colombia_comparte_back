...
export class NoticiaController implements NoticiaService {
  async create(req: Request, res: Response): Promise<any> {
    try {
      ...
      return res.status(500).json({ ok: false, error_message: "Error al crear noticia" });
    }
  }
  ...
  async getAll(req: Request, res: Response): Promise<any> {
    try {
      ...
      return res.status(500).json({ ok: false, error_message: "Error al obtener noticias" });
    }
  }
  ...
  async getById(req: Request, res: Response): Promise<any> {
    try {
      ...
      if (req.user && req.user.rol !== "superadmin" && req.user.pais !== noticia.pais) {
        return res.status(403).json({ ok: false, error_message: "Acceso denegado a esta noticia" });
      }

      return res.status(200).json({ ok: true, noticia });
    } catch (error) {
      console.error("Error getting noticia by id:", error);
      return res.status(500).json({ ok: false, error_message: "Error al obtener noticia" });
    }
  }
  ...
  async update(req: Request, res: Response): Promise<any> {
    try {
      ...
      const updated = await NoticiaModel.findByIdAndUpdate(id, updates, { new: true });
      return res.status(200).json({ ok: true, noticia: updated });
    } catch (error) {
      console.error("Error updating noticia:", error);
      return res.status(500).json({ ok: false, error_message: "Error al actualizar noticia" });
    }
  }
