...
export class TestimonioController implements TestimonioService {
  async create(req: Request, res: Response): Promise<any> {
    try {
      ...
      return res.status(500).json({ ok: false, error_message: "Error al crear testimonio" });
    }
  }
  ...
  async getAll(req: Request, res: Response): Promise<any> {
    try {
      ...
      const items = await TestimonioModel.find(filter).sort({ fecha_creacion: -1 });
      return res.status(200).json({ ok: true, testimonios: items });
    } catch (error) {
      console.error("Error getting testimonios:", error);
      return res.status(500).json({ ok: false, error_message: "Error al obtener testimonios" });
    }
  }
  ...
  async getById(req: Request, res: Response): Promise<any> {
    try {
      ...
      if (!item) return res.status(404).json({ ok: false, error_message: "Testimonio no encontrado" });

      if (req.user && req.user.rol !== "superadmin" && req.user.pais !== item.pais) {
      ...
      return res.status(500).json({ ok: false, error_message: "Error al obtener testimonio" });
    }
  }
  ...
  async update(req: Request, res: Response): Promise<any> {
    try {
      ...
      const updated = await TestimonioModel.findByIdAndUpdate(id, updates, { new: true });
      return res.status(200).json({ ok: true, testimonio: updated });
    } catch (error) {
      console.error("Error updating testimonio:", error);
      return res.status(500).json({ ok: false, error_message: "Error al actualizar testimonio" });
    }
  }
