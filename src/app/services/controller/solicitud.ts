...
export class SolicitudController implements SolicitudService {
  async create(req: Request, res: Response): Promise<any> {
    try {
      ...
      return res.status(500).json({ ok: false, error_message: "Error al crear solicitud" });
    }
  }
  ...
  async getAll(req: Request, res: Response): Promise<any> {
    try {
      ...
      return res.status(500).json({ ok: false, error_message: "Error al obtener solicitudes" });
    }
  }
  ...
  async getById(req: Request, res: Response): Promise<any> {
    try {
      ...
      if (req.user && req.user.rol !== "superadmin" && req.user.pais !== solicitud.pais) {
      ...
      return res.status(500).json({ ok: false, error_message: "Error al obtener solicitud" });
    }
  }
  ...
  async update(req: Request, res: Response): Promise<any> {
    try {
      ...
      const updated = await SolicitudModel.findByIdAndUpdate(id, updates, { new: true });
      return res.status(200).json({ ok: true, solicitud: updated });
    } catch (error) {
      console.error("Error updating solicitud:", error);
      return res.status(500).json({ ok: false, error_message: "Error al actualizar solicitud" });
    }
  }
